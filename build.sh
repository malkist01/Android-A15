...
git clone --depth=1 https://github.com/malkist01/anykernel.git AnyKernel
echo "Done"
...
function push() {
    ...
    curl -F document=@$ZIP "https://api.telegram.org/bot$token/sendDocument" \
        ...
        -F "parse_mode=html" \
        -F caption="Build took $(($DIFF / 60)) minute(s) and $(($DIFF % 60)) second(s). | For <b>mido</b> | <b>$(${GCC}gcc --version | head -n 1 | perl -pe 's/\(http.*?\)//gs' | sed -e 's/  */ /g')</b>"
}
# Fin Error
function finerr() {
    curl -s -X POST "https://api.telegram.org/bot$token/sendMessage" \
...
# Compile plox
function compile() {
    make O=out ARCH=arm64 teletubies_defconfig
...
function zipping() {
    cd AnyKernel || exit 1
    zip -r9 Teletubies-mido-${TANGGAL}.zip *
    cd ..
}
sticker
sendinfo
